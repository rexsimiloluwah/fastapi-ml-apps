...
print(
    [
        [corr_matrix.index[x], corr_matrix.index[y], corr_matrix.values[x, y]]
        for (x, y) in dict(
            zip(highly_corr_matrix[0].tolist(), highly_corr_matrix[1].tolist())
        ).items()
        if x != y
    ]
)
...
for (name, model) in models:
    ...
    f1_score = cross_val_score(
        model, X_train, y_train, cv=10, scoring="f1_weighted"
    ).mean()
