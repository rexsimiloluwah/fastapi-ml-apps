...
LABELS = os.path.join(BASE_DIR,"labels.json")
...
class SSDObjectDetection:
    def __init__(self, save_image=False):
        ...
        print(self.labels)
        self.weights_path = os.path.join(os.path.dirname(__file__),"models","MobileNetSSD_deploy.caffemodel")
        self.prototxt_path = os.path.join(os.path.dirname(__file__),"models","MobileNet_deploy.prototxt")
        self.colors = COLORS
    ...
    def _detect(self, image, display=False):
        ...
        for i in np.arange(detections.shape[2]):
            ...
            if confidence > 0.6:
                ...
                cv2.rectangle(
                    image, (startX, startY), (endX, endY), self.colors[idx], 3
                )
                ...
                self.draw_text(
                    image,
                    label,
                    pos=(startX, startY + 35),
                    text_color_bg=self.colors[idx],
                )
        ...
        if self.save_image:
            ...
            self.output_image_url = cloudinary.uploader.upload(
                os.path.join(OUTPUT_DIR, save_filename)
            )["url"]
...
if __name__ == "__main__":
    ...
    image = cv2.imread(os.path.join(os.path.dirname(__file__),"assets","dog-hug.jpg"))
